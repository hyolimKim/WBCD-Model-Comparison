...
2. **통계적 신뢰도 확보**:  K-Fold와 ROC Curve 시각화를 통해 모델 성능의 일반화 및 안정성을 입증합니다.
...
### 1-1. 모델학습 
* **MLP**
-안정적인 학습을 위해 데이터를 표준화하였습니다.
-100개의 노드를 가진 첫 번째 은닉층과 50개의 노드를 가진 두 번째 은닉층을 구성했습니다.
-반복하는 최대 횟수(에포크)를 학습할 수 있도록 충1000으로 설정하였습니다.

* **RF**
-결정 트리의 개수를 100개로 지정하였습니다.
-feature_importances_ 속성을 통해 특징 중요도를 자체적으로 제공하였습니다.
```python
...
*  특징 중요도 분석 결과, 세 모델 모두에서 'worst' 접두사가 붙은 특징들이 진단에 핵심적인 역할을 함을 확인했습니다.
이는 종양의 최종 상태가 악성/양성 판별에 가장 결정적인 정보임을 시사합니다.
